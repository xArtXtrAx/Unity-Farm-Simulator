...
namespace FarmSimulator.Domain.Inventory
{
    public readonly struct InventorySlot : IEquatable<InventorySlot>
    {
        ...
        {
            return Nullable.Equals(ItemId, other.ItemId) && Quantity == other.Quantity;
        }
        ...
        public override int GetHashCode()
        {
            unchecked
            {
                return ((ItemId?.GetHashCode() ?? 0) * 397) ^ Quantity;
            }
        }
    ...
    {
        public InventorySlotSnapshot(string itemId, int quantity)
        {
            ItemId = itemId;
            Quantity = quantity;
        }
        ...
        public bool Equals(InventorySlotSnapshot other)
        {
        ...
        public override bool Equals(object obj)
        {
            return obj is InventorySlotSnapshot other && Equals(other);
        }
        ...
        public static bool operator ==(InventorySlotSnapshot left, InventorySlotSnapshot right)
        {
            return left.Equals(right);
        }
    ...
    public readonly struct AddItemResult : IEquatable<AddItemResult>
    {
        ...
        public override bool Equals(object obj)
        {
            return obj is AddItemResult other && Equals(other);
        }
        ...
        public override int GetHashCode()
        {
            unchecked
            {
                ...
                hashCode = (hashCode * 397) ^ Requested;
                hashCode = (hashCode * 397) ^ Added;
        ...
        public static bool operator ==(AddItemResult left, AddItemResult right)
        {
            return left.Equals(right);
        }
    ...
    public sealed class InventorySnapshot
    {
        ...
        public InventorySnapshot(
            IEnumerable<InventorySlotSnapshot> slots,
            int selectedIndex)
        {
            if (slots == null)
            {
    ...
    public sealed class InventoryState
    {
        ...
        public InventoryState()
            : this(Array.Empty<InventorySlotSnapshot>(), DefaultSlotCount)
        {
        }

        public InventoryState(int slotCount)
            : this(Array.Empty<InventorySlotSnapshot>(), slotCount)
        {
        }

        public InventoryState(IEnumerable<InventorySlotSnapshot> initialSlots)
            : this(initialSlots, DefaultSlotCount)
        {
        }

        public InventoryState(
            IEnumerable<InventorySlotSnapshot> initialSlots,
            int slotCount)
        {
        ...
        public bool CycleSelection(int delta)
        {
            ...
            int normalizedDelta = delta % SlotCount;
            int nextIndex = (selectedIndex + normalizedDelta + SlotCount) % SlotCount;
            return SelectSlot(nextIndex);
        }
        ...
        {
            return (InventorySlot[])slots.Clone();
        }
        ...
        {
            AssertIndex(index);
            slots[index] = new InventorySlot(itemId, quantity);
        }
        ...
        public void SetSlot(int index, string itemId, int quantity)
        {
            ...
            SetSlot(index, ItemId.Parse(itemId), quantity);
        }
        ...
        public int GetItemCapacity(ItemId itemId)
        {
            ...
            foreach (InventorySlot slot in slots)
            {
                if (slot.IsEmpty)
                {
                    capacity += definition.StackLimit;
                    continue;
                }

                if (slot.ItemId.Value == itemId)
                {
                    capacity += definition.StackLimit - slot.Quantity;
                }
            }
        ...
        public int GetItemCapacity(string itemId)
        {
            return GetItemCapacity(ItemId.Parse(itemId));
        }

        public bool CanAddItem(ItemId itemId, int quantity = 1)
        {
            AssertPositiveQuantity(quantity, "Quantity to add");
            return GetItemCapacity(itemId) >= quantity;
        }

        public bool CanAddItem(string itemId, int quantity = 1)
        {
            return CanAddItem(ItemId.Parse(itemId), quantity);
        }
        ...
        public AddItemResult AddItem(ItemId itemId, int quantity = 1)
        {
            ...
            if (GetItemCapacity(itemId) < quantity)
            {
            ...
            for (int index = 0; index < SlotCount && remaining > 0; index += 1)
            {
                InventorySlot slot = slots[index];
                if (slot.IsEmpty ||
                    slot.ItemId.Value != itemId ||
                    slot.Quantity >= definition.StackLimit)
                {
                    continue;
                }
        ...
        public AddItemResult AddItem(string itemId, int quantity = 1)
        {
            return AddItem(ItemId.Parse(itemId), quantity);
        }

        public bool ConsumeSelected(int quantity = 1)
        {
            return ConsumeFromSlot(selectedIndex, quantity);
        }

        public bool ConsumeFromSlot(int index, int quantity = 1)
        {
            AssertIndex(index);
            AssertPositiveQuantity(quantity, "Quantity to consume");
            ...
            {
                SetSlot(index, slot.ItemId.Value, remaining);
            }
        ...
        public InventorySlotSnapshot[] SnapshotSlots()
        {
            ...
            {
                InventorySlot slot = slots[index];
                snapshotSlots[index] = slot.IsEmpty
                    ? new InventorySlotSnapshot(null, 0)
                    : new InventorySlotSnapshot(slot.ItemId.Value.Value, slot.Quantity);
            }
        ...
        {
            return new InventorySnapshot(SnapshotSlots(), selectedIndex);
        }
        ...
        public void RestoreSlots(IEnumerable<InventorySlotSnapshot> snapshotSlots)
        {
            ...
            {
                throw new ArgumentNullException(nameof(snapshotSlots));
            }
        ...
        private static InventorySlot[] BuildSlots(
            IEnumerable<InventorySlotSnapshot> sourceSlots,
            int slotCount)
        {
            ...
            for (int index = 0; index < source.Length; index += 1)
            {
                ...
                {
                    restoredSlots[index] = InventorySlot.Empty;
                    continue;
                }

                restoredSlots[index] = new InventorySlot(
                    ItemId.Parse(sourceSlot.ItemId),
                    sourceSlot.Quantity);
            }
        ...
        private void AssertIndex(int index)
        {
            if (index < 0 || index >= SlotCount)
            {
                throw new ArgumentOutOfRangeException(
                    nameof(index),
                    index,
                    $"Inventory slot index is outside the valid range: {index}.");
            }
        }

        private static void AssertPositiveQuantity(int quantity, string label)
        {
            if (quantity < 1)
            {
                throw new ArgumentOutOfRangeException(
