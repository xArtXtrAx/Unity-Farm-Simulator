...
namespace FarmSimulator.Tests.EditMode
{
    public sealed class ItemCatalogTests
    {
        ...
        {
            string[] ids = ItemCatalog.All.Select(definition => definition.Id.Value).ToArray();
            Assert.That(ids, Is.EqualTo(new[]
            {
                "hoe", "watering-can", "turnip-seeds", "turnip",
                "potato-seeds", "potato", "radish-seeds", "radish"
            }));
        }
        ...
        [TestCase("potato-seeds", "Semillas de papa", "SP", ItemCategory.Seed, 99, 50, 0)]
        [TestCase("potato", "Papa", "PP", ItemCategory.Crop, 99, 0, 100)]
        [TestCase("radish-seeds", "Semillas de rábano", "SR", ItemCategory.Seed, 99, 75, 0)]
        [TestCase("radish", "Rábano", "RB", ItemCategory.Crop, 99, 0, 150)]
        public void DefinitionMatchesApprovedSourceData(
            string itemId, string expectedName, string expectedShortLabel,
            ItemCategory expectedCategory, int expectedStackLimit,
            int expectedBuyPrice, int expectedSellPrice)
        {
        ...
        {
            string[] purchasable = ItemCatalog.All.Where(definition => definition.IsPurchasable)
                .Select(definition => definition.Id.Value).ToArray();
            Assert.That(purchasable, Is.EqualTo(new[]
            {
                "turnip-seeds", "potato-seeds", "radish-seeds"
            }));
        ...
        {
            string[] sellable = ItemCatalog.All.Where(definition => definition.IsSellable)
                .Select(definition => definition.Id.Value).ToArray();
            Assert.That(sellable, Is.EqualTo(new[] { "turnip", "potato", "radish" }));
        ...
        {
            Assert.That(Enum.GetValues(typeof(ItemCategory)).Cast<ItemCategory>(), Is.EqualTo(new[]
            {
                ItemCategory.Tool, ItemCategory.Seed, ItemCategory.Crop,
                ItemCategory.Material, ItemCategory.Food, ItemCategory.Gift
            }));
        }
        ...
        public void DefinitionRejectsNonPositiveBuyPrice() =>
            Assert.Throws<ArgumentOutOfRangeException>(() =>
                new ItemDefinition(ItemId.TurnipSeeds, "Semillas de nabo", "SN", ItemCategory.Seed, 99, buyPrice: 0));

        [Test]
        public void DefinitionRejectsNonPositiveSellPrice() =>
            Assert.Throws<ArgumentOutOfRangeException>(() =>
                new ItemDefinition(ItemId.Turnip, "Nabo", "NB", ItemCategory.Crop, 99, sellPrice: 0));

        private static int? ToOptionalPrice(int price) => price == 0 ? (int?)null : price;
    }
}
